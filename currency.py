...
def get_src(json):
    ...
    x = introcs.rfind_str(json, "src")
    amt = first_inside_quotes(json[x+4:])
...
def get_dst(json):
    ...
    x = introcs.rfind_str(json, "dst")
    dst = first_inside_quotes(json[x+4:])
...
def iscurrency(currency):
    ...
    # only currency can fail
    x = service_response(currency, 'EUR', 2.5)

    # check for error remember this is true if has error.
    y = has_error(x)
